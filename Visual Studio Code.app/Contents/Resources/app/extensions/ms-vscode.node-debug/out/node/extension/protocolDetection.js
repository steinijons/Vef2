...
function detectProtocolForAttach(config) {
    ...
    return new Promise((resolve, reject) => {
        ...
        return {
            reason: localize(2, null, err.toString()),
            protocol: 'legacy'
        };
    }).then(result => {
        cleanup();
...
function detectProtocolForLaunch(config) {
    if (config.runtimeExecutable) {
        utilities_1.log(localize(3, null));
        return 'inspector';
    }
    else {
        ...
        if (semVerString) {
            ...
            if (semVerStringToInt(config.__nodeVersion) >= InspectorMinNodeVersionLaunch) {
                utilities_1.log(localize(4, null, config.__nodeVersion));
                return 'inspector';
            }
            else {
                utilities_1.log(localize(5, null, config.__nodeVersion));
                return 'legacy';
            }
        }
        else {
            utilities_1.log(localize(6, null));
            return 'inspector';
        }
    }
}
